...
import { OrderData, VerifyCouponData } from "../types";
...
export const verifyCoupon = (data: VerifyCouponData) => api.post(`${ORDER_SERVICE_PREFIX}/coupons/verify`, data)
export const createOrder = (data: OrderData, idempotencyKey: string) => api.post(`${ORDER_SERVICE_PREFIX}/orders`, data, {
    headers: {
        'Idempotency-Key': idempotencyKey
    }
})
export const getSingleOrder = (orderId: string, requiredfields: string) => api.get(`${ORDER_SERVICE_PREFIX}/orders/${orderId}?fields=${requiredfields}`)